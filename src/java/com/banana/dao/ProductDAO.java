...
public class ProductDAO {
    ...
    public List<ProductEntity> viewAllProductByBrand(int offset, int noOfRecords, String brand) {        //
        // mới nhất trước
        String query = "SELECT SQL_CALC_FOUND_ROWS * FROM emarkett.product WHERE brand LIKE N'%" + brand + "%' LIMIT " + offset + ", " + noOfRecords;
        List<ProductEntity> list = new ArrayList<ProductEntity>();
        ProductEntity product = null;
        try {
            Connection connection = connectDB.getMyConnection();
    ...
    public List<ProductEntity> viewAllProductByBrand(int offset, int noOfRecords, String brand, Integer categoryId) {        //
        // mới nhất trước
        String query = "SELECT SQL_CALC_FOUND_ROWS * FROM emarkett.product WHERE brand LIKE N'%" + brand + "%' AND category_category_id = " + categoryId + " LIMIT " + offset + ", " + noOfRecords;
        List<ProductEntity> list = new ArrayList<ProductEntity>();
        ProductEntity product = null;
        try {
            Connection connection = connectDB.getMyConnection();
    ...
    public List<ProductEntity> viewBestSellProduct(int offset, int noOfRecords) {
        String query = "SELECT quantity, p.* FROM emarkett.product as p, (\n"
                ...
                + "order by bestSell.quantity desc LIMIT " + offset + ", " + noOfRecords;
        List<ProductEntity> list = new ArrayList<ProductEntity>();
        ProductEntity product = null;
        try {
            Connection connection = connectDB.getMyConnection();
...
//            System.out.println(rs.getNString("name"));
    ...
    public ProductEntity getProduct(String description) {

        String query = "SELECT * FROM emarkett.product WHERE description = N'" + description + "'";
        ProductEntity product = null;
        try {
            Connection connection = connectDB.getMyConnection();
            Statement statement = connection.createStatement();
    ...
    public Integer getProductId(){
        ResultSet rs;
        try {
            rs = connectDB.getMyConnection().createStatement().executeQuery("SELECT product_id FROM emarkett.product order by product_id desc limit 1");
            if (rs.next()) {
                return rs.getInt(1);
            }
...
////        List<ProductEntity> list = dao.viewAllProductByBrand(0, 5, "asus", 1);
...
////        productDetailEntity.setInformation("gì đó");
////        productDetailEntity.setProductId(dao.getProductId());
//        pddao.insertProductDetail(dao.getProductId(), "gì đó2", null, null, null, null, null, null, null);
    }
}
